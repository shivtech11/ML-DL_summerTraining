...
def predict(coef, row){
        ...
        return float(1/(1+math.exp(-y_pred)))
        }
...
def evaluateAlgorithm(dataset, epoachs, learning_rate){
        scores = []
        ...
        for fold in folds:
            ...
            for row in fold:
                ...
                rowcopy[-1] = None
                test.append(rowcopy)
            pred = logisticRegression(train, test, epoachs, learning_rate)
            actual = [row[-1] for row in fold]
            score = accuracy(actual, pred)
            scores.append(score)
        return scores
        }

def stochasticGradient(dataset, epoachs, learning_rate){
...
def logisticRegression(train, test, epoachs, learning_rate){
        predictions = []
        coef = stochasticGradient(train, epoachs, learning_rate)
        for row in test:
            y_pred = predict(coef, row)
